...
def removedups(ll):
    ...
    while curr.next is not None:
        if curr.item == curr.next.item:
            temp = curr.next.next       #preserve the next ptr of the node with dup val
            curr.next = temp            #then adjust the next ptr of the current node
            ll.size -= 1
        else:
            curr = curr.next            #keep traversing
