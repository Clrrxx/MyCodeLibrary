...
#write your codes here
    ...
    for i in range(TABLESIZE):
        ...
        if slot.indicator == EMPTY:
            insert_idx = candidate if candidate != -1 else idx
            hash_table[insert_idx].key = key
            hash_table[insert_idx].indicator = USED
            return comparisons
        ...
        if slot.key == key:
            ...
            if slot.indicator == DELETED and candidate == -1:
                candidate = idx
...
#write your codes here
    ...
    for i in range(TABLESIZE):
        ...
        if slot.key == key and slot.indicator == USED:
            hash_table[idx].indicator = DELETED
            return comparisons
