...
#add your implementations
    ...
    def dfs(node, curr_word):
        ...
        if node.is_end_of_word:         #we reach the end of the tree
            if L <= curr_word <= R:
                count[0] += 1              #if word is between these 2 lexigraphically, increment count

        child = node.first_child

        while child:
            next = curr_word + child.char   #this builds the word letter by letter

            if next > R:
                break                       #if next > R (CAT > CA) then break, if next is greater than any prefix after will naturally be larger
